      ...
      if (medRes.ok) {
        ...
        if (medData && medData.items && medData.items.length > 0) {
          ...
          if (elMedDate && elMedTitle && elMedBody) {
            ...
            elMedDate.textContent = `${d.getDate()} de ${meses[d.getMonth()]}, ${d.getFullYear()}`;
          ...
          if (elMedList) {
            ...
            sortedMeds.forEach(m => {
              ...
              const dateStr = `${d.getDate()} de ${meses[d.getMonth()]}, ${d.getFullYear()}`;
              listHtml += `
                <div class="med-entry" style="margin-bottom:15px;">
                  <div class="med-date-badge">${dateStr}</div>
                  <div class="med-title">${m.title}</div>
                  <div class="med-body">${m.body}</div>
                </div>
              `;
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOTE: Carga la canci�n desde assets/ambient.mp3.
